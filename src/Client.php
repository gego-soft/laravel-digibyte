...
class Client
{
    ...
    public function __construct($config = [])
    {
        ...
        $handlerStack->push(
            Middleware::mapResponse(function (ResponseInterface $response) {
                return DigibytedResponse::createFrom($response);
            }),
            'json_response'
        );
    ...
    /**
     * Makes request to digibyte Core.
     *
     * @param string $method
     * @param mixed  $params
     *
    ...
    public function request($method, $params = [])
    {
          ...
          if(count($params)>0)
           {
            ...
            if ($response->hasError()) {
                ...
                throw new Exceptions\DigibytedException($response->error());
            }
            ...
            if (
                ...
                throw new Exceptions\DigibytedException($exception->getResponse()->error());
            }
        ...
        } catch (Exceptions\DigibytedException $exception) {
            throw $exception;
        }
    }

    /**
     * Makes async request to digibyte Core.
     *
     * @param string        $method
     * @param mixed         $params
        ...
        callable $onRejected = null)
    {
        ...
        $promise->then(
            function (ResponseInterface $response) use ($onFullfiled) {
                $error = null;
                if ($response->hasError()) {
                    $error = new Exceptions\DigibytedException($response->error());
                }
            ...
            function (RequestException $exception) use ($onRejected) {
                if (
                    ...
                    $exception = new Exceptions\DigibytedException(
    ...
    /**
     * Converts amount from satoshi to digibyte.
     *
     * @param int $amount
     *
     * @return float
    ...
    /**
     * Converts amount from digibyte to satoshi.
     *
     * @param float $amount
     *
     * @return int
